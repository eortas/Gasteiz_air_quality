...
function renderDashboard3() {
  ...
  const labels = [...historicalLabels, currentLang === 'es' ? 'Mañana' : 'Bihar'];
  const activeTarget = targetsData[`${currentContV10}_${currentZoneV10}_d1`] || {};
  const activePrediction = activeTarget.prediction;
  const currentForecast = Array(labels.length).fill(null);
  let lastObservedIndex = -1;

  for (let i = d.real.length - 1; i >= 0; i--) {
    if (d.real[i] !== null && d.real[i] !== undefined) {
      lastObservedIndex = i;
      break;
    }
  }

  if (lastObservedIndex >= 0 && activePrediction !== undefined) {
    currentForecast[lastObservedIndex] = d.real[lastObservedIndex];
    currentForecast[labels.length - 1] = activePrediction;
  }
  ...
  perfChart = new Chart(ctx, {
      ...
      data: {
          labels: labels,
          datasets: [
              { label: t.backReal, data: [...d.real, null], borderColor: getCssVar('--observed'), backgroundColor: 'transparent', borderWidth: 2, pointRadius: 4, tension: 0.4 },
              { label: t.backPred, data: [...d.pred, null], borderColor: getCssVar('--accent'), backgroundColor: 'transparent', borderDash: [5,5], borderWidth: 2, pointRadius: 4, tension: 0.4 },
              { label: t.backCurrentPred, data: currentForecast, borderColor: getCssVar('--accent'), backgroundColor: 'transparent', borderDash: [2,4], borderWidth: 3, pointRadius: 5, tension: 0.2 }
          ]
      },
